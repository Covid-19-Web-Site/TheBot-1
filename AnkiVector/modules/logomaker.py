...
async def lego(event):
    ...
    try:
        ...
        fname2 = "SallyLogo.png"
        img.save(fname2, "png")
        await tbot.send_file(event.chat_id, fname2, caption="Made By @NipunGroupBot 🌹")
        if os.path.exists(fname2):
            os.remove(fname2)
    except Exception as e:
        await event.reply(f"Error Go to Help From  @TheSallySupportl, {e}")
...
async def lego(event):
    ...
    try:
        ...
        await event.reply(f"Error Go to Help From  @TheSallySupport, {e}")
...
async def lego(event):
    ...
    try:
        ...
        y = (image_heightz - h) / 2 + 6
        draw.text(
            (x, y), text, font=font, fill="black", stroke_width=25, stroke_fill="red"
        )
        fname2 = "SallyrLogo.png"
        img.save(fname2, "png")
        await tbot.send_file(event.chat_id, fname2, caption="Made By @NipunGroupBot 🌹")
        if os.path.exists(fname2):
            os.remove(fname2)
